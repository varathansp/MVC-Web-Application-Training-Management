    ...
    public class AdminDAL
    {
        ...
        public int AddCourse( string courseName, string DName, DateTime SDate, DateTime EDate, string Session, string TID)
        {
            ...
            try
            {
                int flag1 = 1;
                int flag2 = 1;
            List<Cours> TrainerCourses = (from data in contextObject.Courses /*where data.TrainerID == TID*/ select data).ToList();
            foreach(Cours data in TrainerCourses)
            {
                flag1++;
